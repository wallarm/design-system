...
import { EditingProvider, useEditingContext } from '../QueryBarInput/QueryBarChip/EditingContext';
import { QueryBarChip } from '../QueryBarInput/QueryBarChip/QueryBarChip';
...
describe('QueryBarChip with EditingContext', () => {
  it('renders in read mode when no editing context', () => {
    render(<QueryBarChip attribute='IP Address' operator='is' value='1.2.3.4' />);
  ...
  it('renders in read mode when editing context targets a different chip', () => {
    render(
      <EditingProvider
        ...
        onSegmentEditBlur={noop}
      >
        <QueryBarChip chipId='chip-0' attribute='IP Address' operator='is' value='1.2.3.4' />
      </EditingProvider>,
    );
    expect(screen.getByText('IP Address')).toBeInTheDocument();
    expect(screen.queryByRole('textbox')).not.toBeInTheDocument();
  ...
  it('does not show input when chipId is not provided', () => {
    render(
      <EditingProvider
        ...
        onSegmentEditBlur={noop}
      >
        <QueryBarChip attribute='IP Address' operator='is' value='1.2.3.4' />
      </EditingProvider>,
    );
    expect(screen.queryByRole('textbox')).not.toBeInTheDocument();
  });
