...
test.describe('Component: QueryBar - Self-Contained Mechanics', () => {
  ...
  test.describe('Interactions', () => {
    ...
    test('Should delete chip when delete button clicked', async ({ page }) => {
      ...
      const deleteButton = page.locator('[data-slot="query-bar-chip-delete"]');
    ...
    test('Should create two conditions with AND chip between them', async ({ page }) => {
      ...
      const firstChip = page.locator('[data-slot="query-bar-chip"]').filter({ hasText: 'Status' });
      await expect(firstChip).toBeVisible();

      await createChipWithSelection(page, 'Priority', 'Low');

      const allChips = page.locator('[data-slot="query-bar-chip"]');
      await expect(allChips).toHaveCount(3);

      const andChip = page.locator('[data-slot="query-bar-chip"]').filter({ hasText: 'AND' });
    ...
    test('Should toggle AND to OR when connector chip clicked', async ({ page }) => {
      ...
      const orChip = page.locator('[data-slot="query-bar-chip"]').filter({ hasText: 'OR' });
    ...
    test('Should remove connector when condition is removed', async ({ page }) => {
      ...
      const statusChip = page.locator('[data-slot="query-bar-chip"]').filter({ hasText: 'Status' });
      await statusChip.hover();
      await page.waitForTimeout(200);
      const deleteButton = statusChip.locator('[data-slot="query-bar-chip-delete"]');
      await deleteButton.click();
      await page.waitForTimeout(300);
