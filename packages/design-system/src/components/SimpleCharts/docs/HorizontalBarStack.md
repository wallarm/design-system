...
- **Click-to-filter** — attach `onSelect`; legend items expose `role='button'` + `tabIndex=0`, and Enter/Space dispatch the same callback. The caller filters (e.g. sets `selectedNames`, or filters `data`). Clicking the active item again to clear ("remove filter") is a caller convention.
