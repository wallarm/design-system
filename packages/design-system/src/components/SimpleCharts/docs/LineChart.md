...
| Hover (point)   | Pointer over the plot body                                                       | Recharts' tooltip state snaps to the nearest X. The Y-locked popover renders one row per visible series at that index. The dashed vertical cursor guideline tracks the cursor X — this is the only on-plot hover affordance; no per-point dots render (`dot={false}` and `activeDot={false}`). Sibling charts that share a `syncId` mirror the highlight automatically. |
...
| Zoom-dragging   | Pointer down → drag on the plot while `<LineChartZoomBrush>` is mounted          | Borderless gray `<ReferenceArea>` (`fill: var(--color-states-primary-hover)`, no stroke) follows the cursor; a floating popover tracks the cursor with the live range text. Hover popover and dashed cursor guideline are suppressed — the zoom popover owns the cursor. |
| Zoom-pending    | Mouse release after a non-zero drag                                              | Selection rectangle stays; popover becomes interactive with the "Zoom in" button. Hover popover stays suppressed until the pending state resolves. Click outside / Escape dismisses; Enter or button click confirms. |
...
| SinglePoint     | `data.length === 1`                                                              | Nothing geometric renders — `dot={false}` + `activeDot={false}` mean no marker, and a one-point line has zero length. Hover still surfaces the value via the dashed cursor + popover. |
...
- **Single point (`data.length === 1`).** Recharts renders nothing geometric — `dot={false}` and `activeDot={false}` mean there is no marker, and a one-point line has zero length. Hover still works (the dashed cursor + popover surface the value). Consider falling back to a different chart type for single-point data.
...
- **Brush popover positioning.** The popover is `position: fixed` portalled to `document.body`. **X tracks the cursor** (`clientX` from the latest mousemove, offset right of the pointer); **Y is locked to the plot-area centre** via recharts' `usePlotArea()` + the recharts surface's viewport rect, then offset up by half its own height (`translateY(-50%)`) so the popover's vertical centre matches `<LineChartTooltip>`. Before recharts has measured the plot, Y falls back to the current pointer `clientY` for the one or two frames until `plotArea` lands. While the popover is open the chart subscribes to `window` `scroll` and `resize` (capture phase) and bumps a render tick, so the memoised viewport rect re-reads — without that subscription the portal's `position: fixed` would pin the popover to the original release point as the page scrolls and the chart would slide away beneath it. During **pending** the captured `clientX` is offset by the chart's horizontal scroll delta (the chart's viewport-`left` at release vs. now) so the popover stays anchored to the plot column, not the viewport.
...
- **Edge tick text anchor uses the original `payload.coordinate`, not `props.x`/`props.y`.** Recharts pre-shifts the first/last tick's `tickCoord` inward so a *middle-anchored* label cannot overflow the plot. Our edge ticks override the anchor to `start` / `end`, so that shift would float the text edge ~10px off its tick (e.g. `23:00` landing visually near the `22:00` band). `renderAxisTick` re-anchors to `payload.coordinate` whenever the caller overrides the anchor, so the text's leading/trailing edge lands directly under the tick.
- **`LineChartXAxis` defaults `interval` to `'preserveStartEnd'`.** Recharts' own default (`preserveEnd`) drops the first data tick whenever it collides with a neighbour, leaving the leftmost label visibly indented from the plot's left edge. We force both edges to render so `index === 0` and `index === visibleTicksCount - 1` are always the actual data edges — which is what the `renderXAxisTick` anchor override assumes when it pins the first label flush-left and the last flush-right.
