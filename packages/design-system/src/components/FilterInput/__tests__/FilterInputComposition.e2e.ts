...
test.describe.skip('Component: FilterInput - Self-Contained Mechanics', () => {
  ...
  test.describe('Interactions', () => {
    ...
    test('Should complete full autocomplete flow: field → operator → value → chip', async ({
      ...
      await page.waitForSelector('[data-slot="filter-input-field-menu"]', {
        state: 'visible',
        timeout: 2000,
      });
    ...
    test('Should delete chip when delete button clicked', async ({ page }) => {
      ...
      const deleteButton = page.locator('[data-slot="filter-input-chip-delete"]');
    ...
    test('Should create two conditions with AND chip between them', async ({ page }) => {
      ...
      const allChips = page.locator('[data-slot="filter-input-chip"]');
      await expect(allChips).toHaveCount(3);

      const andChip = page.locator('[data-slot="filter-input-chip"]').filter({ hasText: 'AND' });
    ...
    test('Should toggle AND to OR when connector chip clicked', async ({ page }) => {
      ...
      const orChip = page.locator('[data-slot="filter-input-chip"]').filter({ hasText: 'OR' });
    ...
    test('Should remove connector when condition is removed', async ({ page }) => {
      ...
      await page.waitForTimeout(200);
      const deleteButton = statusChip.locator('[data-slot="filter-input-chip-delete"]');
      await deleteButton.click();
      await page.waitForTimeout(300);
    ...
    test('Should open field menu when clicking empty space after chips', async ({ page }) => {
      ...
      const filterInput = page.locator('[data-slot="filter-input"]');
      const filterInputBox = await filterInput.boundingBox();
      expect(filterInputBox).toBeTruthy();

      // Click near the right edge of the FilterInput (empty space after input)
      await page.mouse.click(
        filterInputBox!.x + filterInputBox!.width - 60,
        filterInputBox!.y + filterInputBox!.height / 2,
      );
