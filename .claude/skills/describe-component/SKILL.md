...
**Write for prototyping, not production.** These docs steer AI that's building *prototypes* to hand to engineering. So frame any backend / data / infra dependency as something to **stub or hardcode for the prototype** (a hardcoded config array, a logged callback) — never as a prerequisite that gates reaching for the component. Over-stressing "it needs a backend" makes the AI think it can't use the component at all; the production wiring is a downstream, engineering concern. (`FilterInput`: a prototype hardcodes `fields` and reads `onChange` — the query endpoint comes later.)
...
**Reading the node (remote Figma MCP):** `get_design_context` can hard-fail with *"nothing selected"* (it wants a live canvas selection even given a valid `nodeId`+`fileKey`), and `get_metadata` on a full **spec page** overflows the token limit (it packs Documentation + Notes + Anatomy + the whole variant matrix). Selection-free path that works: call `get_metadata` for the frame tree (when it's too big it saves to a file — `jq -r '.[].text'` + grep for the `Documentation` / `Notes` frame node IDs), then `get_screenshot(nodeId)` on just those frames and read the prose from the image (`curl` the returned URL → Read the PNG). **A tall, multi-section content/notes frame won't read at full size** — screenshot its **child sections individually** at `maxDimension ≈ the frame's width` (1:1 native), not the whole frame at a big `maxDimension`, or the text scales below readability. On a **very** large page (tens of thousands of px — big pattern specs) `get_metadata` can **time out entirely**, not just overflow, so you can't get child node IDs at all: don't burn the run chasing it — accept you won't read the Notes prose, lean on the Code Connect `example()`s + the stories + a quick paste from the designer, and move on. **Get ahead of the timeout when you already know a page is huge: ask the designer up front for the node-id of just the `Notes` / `Documentation` *sub-frame* (a small node screenshots readably even when the parent times out), and treat "paste the Notes text" as a first-class fallback, not a last resort.** **Two-file gotcha (desktop-bridged MCP):** it reads only the **currently-active tab** — when a component spans two files (a Foundations text-style file + a Components file, e.g. documenting `Code` alongside `CodeSnippet`), a node in the *non-active* file fails with *"make sure the document is the active tab"*; tell the designer which file to bring to the front and read one file at a time.
...
| external spec (Confluence / PRD), if one exists | the canonical behavior spec behind the component — reconcile the doc with it, and flag **spec-ahead-of-code** (a format/behavior the spec describes but the component doesn't ship — e.g. a display guide's Duration / Countdown) the same way you flag Figma-ahead |
| **consuming app repo** — *host / shell / platform / app-frame components only* | how the platform actually **mounts and uses** it (mounted once? feature-props vs composed slots? first-load-only animation?) — the authoritative usage source the DS code / Figma can't express. Ask the designer where it's consumed; it's often a sibling repo (e.g. `core-frontend` for `AppShell` / `RemoteShell`) |
...
- **Preempt the nearest adjacent need.** For a value/text control, name the closest sanctioned path for the obvious *next* ask (multi-value, creatable / typeahead, masked, free-text) — or say plainly that none ships. A doc that's silent on the adjacent need reads as a gap, and the AI will invent one rather than reach for the right neighbour.
...
| **GitLab Pajamas** — design.gitlab.com | Developer / DevOps tooling, data-dense UI (tables, charts, empty states); ships explicit **when-to-use / when-not-to-use** + do/don't sections — the exact shape we write, and one of the few that's fetchable |

This roster isn't fixed at these — add reference systems as they prove useful in practice. Where our system **differs** from a reference *either way* — stricter (Nord allows non-interactive tags; ours are always a Badge) **or more permissive** (EUI shows one toast at a time; our `Toaster` stacks 3 + hover-expand) — document **our shipped behavior** and cite the divergence; never import the reference's rule over ours. For a genuinely agnostic UX pattern (a filter/query builder, a date picker), a plain `WebSearch` for the *pattern* (not the component name) is fair game and surfaces the when/when-not framing — cite it.

**Fetchability** (so the consult step doesn't silently degrade to "announce + fail"): Nord ✓ and **GitLab Pajamas ✓** via plain `WebFetch` (Pajamas detail pages are server-rendered — their `when to use` / `when not to use` prose comes through, and the docs are open-source as a fallback); Carbon / Atlassian / Primer ✗ (JS-rendered — fetches return nav shells with no usage content). Fall back to their GitHub doc sources or an `llms.txt` when a fetch comes back empty.
...
- **Don't blend shipped-reality with likely-intent in one breath.** When a Locked rule has an intended-but-unbuilt variant, state the can't-do-today decisively (no prop, don't hand-hack it) and keep "intended but unbuilt" as a *separate* parenthetical — a bullet that reads as both "locked" and "flagged for FE" lands as optional permission, and the AI splits on it.
- **Confirm the component's *category* before drafting when it's ambiguous.** A component can sit in more than one bucket (`Dialog` is a *layout* — peer of Drawer / full-page — not "the blocking message"); surface your assumed framing to the designer early, because a recategorization changes the lead, the boundaries, and the "reach for it when." **Confirm the doc↔code mapping too:** when the designer keeps several Figma pages/components for what the code ships as **one** top-level directory (Select: button / input / menu pages → one `Select/` dir), the doc maps to the *code* — one dir = one file — and the Figma pages become *renditions*; lead with a which-rendition router rather than splitting into files that can't take effect. Some entries are **patterns**, not components (heavyweight, infra-backed, filed under Storybook `Patterns/` — e.g. `FilterInput`); name that framing up front.

### 4 · Sanity check (before sign-off)
Hand a fresh subagent **only** the drafted `.llm.md` — no code, Figma, or Storybook — plus 5–7 deliberately tricky prompts targeting the riskiest calls: boundary traps ("show a GraphQL operation type"), locked-rule traps ("make DELETE red"), pairing, sizing, a **creatable / free-entry** trap for any value-picker ("type a value that isn't in the list"), and — when the component carries copy rules — a microcopy trap ("write the toast title for X"). For a **two-axis style matrix** (e.g. emphasis × intent), target axis *combinations* — off-grid cells, "which B for this A" — not single-axis lookups, and answer them in the doc with a worked recipe per common scenario (main / secondary / destructive / dark-surface); per-axis prose alone tests MED. **For a family-router or otherwise high-stakes file, run the check as a small panel (~3 judges on the same doc + prompts) and weight a flag by convergence** — unanimous = real must-fix, lone = likely nit; a single judge is enough for an inherited family sibling. **Tell the agent up front the doc is judgment-only** — the prop/type/variant/default contract lives in the code + MCP, so "no prop list / missing types / no API signature" is *out of scope*, not a weakness; it should flag only wrong or missing *judgment*. (Running several blind lenses in parallel? Give each the FULL doc verbatim — an abridged copy yields false "missing section" flags.) For each prompt it reports: decision · which section drove it · confidence (HIGH/MED/LOW). Fix every MED/LOW with small, targeted additions. Aim for all-HIGH before showing the designer.

**4b · Code-aware pass.** After the blind check passes, run one quick pass against the component source asking only: *"does any doc claim contradict the code?"* (defaults, prop names, behavior). The blind check can't see doc-vs-code drift — this catches it. **Two things the panel structurally can't catch, so 4b owns them:** (1) a *family* claim ("Input / Textarea / NumberInput all read `Field` context") — verify **each** member in code, because grounding lumps siblings and the panel rates the plausible-but-wrong claim HIGH; (2) any code defect you're about to chip — confirm it in the source first (grounding can pattern-match a *false* bug off a sibling), so 4b doubles as the bug-confirmation gate before you spawn a fix. Revisions to an existing `.llm.md` go through steps 4–4b exactly like fresh files.
...
**Documenting a *new* family that shares an engine/rules?** Do the **anchor** first — the member that exercises the shared machinery — with the full panel, confirm the shape with the designer, then **fan out** the siblings on that validated shape with lighter (single-judge) checks: faster, and one voice by construction (the date/time family — `DateInput` anchor → `TimeInput` / `DateRangeInput` inherited the value-type, provider, and Field-gap rules, then `Calendar` joined as the popover member).
